...
impl Triangle {
    fn new(x1: f32, y1: f32, x2: f32, y2: f32, x3: f32, y3: f32) -> Box<dyn Shape> {
...
impl Shape for Triangle {
    ...
    fn area(&self) -> f32 {
        let area = 0.5 * ((self.x1 - self.x3) * (self.y2 - self.y1) - (self.x1 - self.x2) * (self.y3 - self.y1));
        area
    }
...
fn main() {
    let circle1 = Circle::new(10, 10, 30.0);
    println!("Original: {}", circle1.rep_string());
    println!("{}", circle1.area());
    let circle2 = circle1.cloner();
    println!("Clone: {}", circle2.rep_string());
