...
fn save_layer(writer: impl Write, pt_list: &[Layer]) {
    let mut writer = csv::WriterBuilder::new()
                        ...
                        .from_writer(writer);
    writer.write_record(["Name", "Color", "Circle (x,y,radius)"]).unwrap(); 
...
fn save_html_layer(writer: impl Write, pt_list: &[Layer]) {
    ...
    writeln!(writer, "<title>Layers</title>").unwrap();
    writeln!(writer, "</head>").unwrap();
    writeln!(writer, "<body>").unwrap();
    writeln!(writer, "<h1>Layers</h1>").unwrap();

    // Create a table for the layers
    writeln!(writer, "<table border=\"1\">").unwrap();
    writeln!(writer, "<tr><th>Name</th><th>Color</th><th>Circles</th></tr>").unwrap();

    for layer in pt_list {
        // Write each layer as a row in the table
        writeln!(writer, "<tr>").unwrap();
        writeln!(writer, "<td>{}</td>", layer.name).unwrap();
        writeln!(writer, "<td>{}</td>", layer.color).unwrap();
        writeln!(writer, "<td>").unwrap();

        for circle in &layer.circles {
            // Write each circle as a list item
            writeln!(writer, "Circle (x={}, y={}, radius={})", circle.x, circle.y, circle.radius).unwrap();
        }
